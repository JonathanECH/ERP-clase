  ...
  <div>
    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <v-table>
          ...
          <tbody>
            <tr v-for="(p, idx) in partidas" :key="idx">
              <td>
                <v-select v-model="p.cuenta" :items="cuentas" variant="outlined" density="compact" hide-details />
              </td>
              <td>
                <v-text-field v-model.number="p.debe" type="number" variant="outlined" density="compact" hide-details
                  :disabled="p.haber > 0" />
              </td>
              <td>
                <v-text-field v-model.number="p.haber" type="number" variant="outlined" density="compact" hide-details
                  :disabled="p.debe > 0" />
              </td>
              <td>
        ...
        <!-- VALIDACIÓN -->
        <v-alert :type="esValido ? 'success' : diferencia === 0 ? 'info' : 'error'" variant="tonal" class="mt-4">
          <template v-if="esValido">Asiento cuadrado - Listo para registrar</template>
          <template v-else-if="diferencia === 0">Ingresa un concepto para registrar</template>
